...
class ModChooseWindow(private val version: Version, private val mainStage: Stage) {
  ...
  fun setChooseWin() {
    ...
    cancelFabricAPI.onAction = EventHandler {
      try {
	      (version.modDownloadInfo as? FabricDonwloadInfo).noNull().apiDownloadTask = null
        fabricAPI.text = Tips.FABRIC_API_NO
      } catch (e: NullPointerException) {
    ...
    if (version.isForge) {
	    forge.text = "Froge : " + (version.modDownloadInfo as? ForgeDownloadInfo).noNull().modVersion
    ...
    if (version.isFabric) {
	    fabric.text = "Fabric : " + (version.modDownloadInfo as? FabricDonwloadInfo).noNull().modVersion
      forge.text = "Forge : 与Fabric不兼容"
      downloadFabricAPI.isDisable = false
      downloadForge.isDisable = true
	    (version.modDownloadInfo as? FabricDonwloadInfo).noNull().also {
		    if (it.isHasApiDownloadTask) {
			    fabricAPI.text = it.apiDownloadTask.noNull().fabricAPIVersionNumber
		    }
	    }
    } else {
      fabric.text = Tips.FABRIC_NO
    ...
    if (version.isQuilt) {
	    quilt.text = "Quilt : " + (version.modDownloadInfo as? QuiltDownloadInfo).noNull().modVersion
