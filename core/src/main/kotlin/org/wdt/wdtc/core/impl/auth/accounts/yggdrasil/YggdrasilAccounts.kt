...
class YggdrasilAccounts(
	...
	private val postUrl = URL("$url/api/yggdrasil/authserver/authenticate")
	
	private val sendPostWithJson: Deferred<UserInformation> = ioAsync {
		postUrl.openConnection().apply {
	...
	override suspend fun getUser(): User = runOnIO {
		...
		sendPostWithJson.await().let {
			User(
				userName, it.accessToken, AccountsType.YGGDRASIL, it.selectedProfile.getString("id"), headFile.await()
			).apply {
