...
class HomeWindow {
  ...
  fun setHome(mainStage: Stage) {
    ...
    val versionSetting = JFXButton("版本设置").apply {
      ...
      if (version != null) {
        isDisable = false
        val windows = VersionSettingWindow(version!!, mainStage)
        onAction = EventHandler { windows.setWindow() }
      } else {
        isDisable = true
      }
    }
    ...
    val launchGameButton = JFXButton().apply {
      text = if (version != null) "启动游戏${version!!.versionNumber}" else "当前无游戏版本"
        ...
        if (version != null) {
          if (isExistUserJsonFile) {
            TaskWindow(
              ...
              taskList.add(launchGame(version.ckeckIsNull()))
              showTaskStage()
            }
    ...
    if (!isExistUserJsonFile) {
      javafxCoroutineScope.launch("Show user window".toCoroutineName()) {
        noUser(mainStage)
      }
    }
  }
  ...
  companion object {
    ...
    private fun launchGame(version: Version): TaskManger {
      return TaskManger("启动游戏", TaskKind.COROUTINES, ioCoroutineScope.launch(
        "${version.versionNumber} launch task".toCoroutineName(), CoroutineStart.LAZY
      ) {
        try {
          LaunchProcess(LaunchGame.create(version).launchTaskProcess) {
