...
public class VersionSettingWindow extends SettingManger {
    private static final double layoutX = 10;
    private static final Logger logmaker = WdtcLogger.getLogger(VersionSettingWindow.class);
    private final Launcher launcher;
    private final DefaultGameConfig.Config config;
    private final WindwosSizeManger size;
    private final Stage MainStage;

    public VersionSettingWindow(Launcher launcher, Stage MainStage) {
    ...
    private void setModPane(ModUtils.KindOfMod kind, AnchorPane ModPane) {
        ...
        if (launcher.getKind() == kind) {
            ModDownloadInfoInterface info = ModUtils.getVersionModInstall(launcher, kind);
            if (info != null) {
