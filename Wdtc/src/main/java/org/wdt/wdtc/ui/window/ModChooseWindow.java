...
public class ModChooseWindow {
    private static final Logger logmaker = WdtcLogger.getLogger(ModChooseWindow.class);
    private final Launcher launcher;
    private final Stage MainStage;
    private final WindwosSizeManger size;

    public ModChooseWindow(Launcher launcher, Stage MainStage) {
    ...
    public void setChooseWin() {
        ...
        DownloadForge.setOnAction(event -> {
            try {
                ModChooseVersionWindow Choose = new ModChooseVersionWindow(ModUtils.KindOfMod.FORGE, MainStage, launcher);
        ...
        DownloadFabric.setOnAction(event -> {
            try {
                ModChooseVersionWindow Choose = new ModChooseVersionWindow(ModUtils.KindOfMod.FABRIC, MainStage, launcher);
                Choose.setModChooser();
            } catch (IOException e) {
                ExceptionWindow.setErrorWin(e);
            }
        ...
        CancelFabric.setOnAction(event -> {
            try {
                launcher.cleanKind();
                launcher.fabricModInstallInfo.setApiDownloadTask(null);
        ...
        DownloadFabricAPI.setOnAction(event -> {
            try {
                ModChooseVersionWindow Choose = new ModChooseVersionWindow(ModUtils.KindOfMod.FABRICAPI, MainStage, launcher);
                Choose.setModChooser();
            } catch (IOException e) {
                ExceptionWindow.setErrorWin(e);
            }
        ...
        DownloadQuilt.setOnAction(event -> {
            try {
                ModChooseVersionWindow Choose = new ModChooseVersionWindow(ModUtils.KindOfMod.QUILT, MainStage, launcher);
                Choose.setModChooser();
            } catch (IOException e) {
                ExceptionWindow.setErrorWin(e);
            }
        ...
        if (ModUtils.gameModIsForge(launcher)) {
            Forge.setText("Froge : " + launcher.forgeModDownloadInfo.getModVersion());
        ...
        if (ModUtils.gameModIsFabric(launcher)) {
            Fabric.setText("Fabric : " + launcher.fabricModInstallInfo.getModVersion());
            Forge.setText("Forge : 与Fabric不兼容");
            DownloadFabricAPI.setDisable(false);
            DownloadForge.setDisable(true);
            if (launcher.fabricModInstallInfo.isAPIDownloadTaskNoNull()) {
                FabricAPI.setText(launcher.fabricModInstallInfo.getApiDownloadTask().getFabricAPIVersionNumber());
            }
        } else {
            Fabric.setText(Tips.FabricNo);
            DownloadFabricAPI.setDisable(true);
        ...
        if (ModUtils.gameModIsQuilt(launcher)) {
            Quilt.setText("Quilt : " + launcher.quiltModDownloadInfo.getModVersion());
        ...
        back.setOnAction(event -> GameVersionListWindow.setWindowScene(MainStage));
        confirm.setOnAction(event -> {
            GameDownloadingWindow gameDownloadingWindow = new GameDownloadingWindow(launcher);
